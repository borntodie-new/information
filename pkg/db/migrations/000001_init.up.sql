CREATE TABLE "users" (
                         ...
                         "last_login_at" timestamptz NOT NULL DEFAULT ('0001-01-01'),
...
COMMENT ON COLUMN "users"."last_login_at" IS '最后一次登陆时间';

COMMENT ON COLUMN "users"."created_at" IS '创建时间';

COMMENT ON COLUMN "users"."updated_at" IS '修改时间';

COMMENT ON COLUMN "user_fans"."follower_id" IS '关注者ID';

COMMENT ON COLUMN "user_fans"."followed_id" IS '被关注者ID';

COMMENT ON COLUMN "user_fans"."created_at" IS '创建时间';

COMMENT ON COLUMN "user_fans"."updated_at" IS '修改时间';

COMMENT ON COLUMN "news"."title" IS '新闻标题';

COMMENT ON COLUMN "news"."source" IS '新闻来源';

COMMENT ON COLUMN "news"."digital" IS '新闻摘要';

COMMENT ON COLUMN "news"."content" IS '新闻内容';

COMMENT ON COLUMN "news"."clicks" IS '新闻浏览量';

COMMENT ON COLUMN "news"."status" IS '0-通过,1-审核中,2-不通过';

COMMENT ON COLUMN "news"."reason" IS '不通过的理由';

COMMENT ON COLUMN "news"."images" IS '新闻图片,序列化后的串';
...
COMMENT ON COLUMN "categories"."updated_at" IS '修改时间';

COMMENT ON COLUMN "categories"."created_at" IS '创建时间';

COMMENT ON COLUMN "collections"."user_id" IS '收藏者的用户ID';

COMMENT ON COLUMN "collections"."news_id" IS '收藏的新闻ID';

COMMENT ON COLUMN "collections"."updated_at" IS '修改时间';

COMMENT ON COLUMN "collections"."created_at" IS '创建时间';

COMMENT ON COLUMN "comments"."content" IS '评论内容';

COMMENT ON COLUMN "comments"."likes" IS '点赞人数';

COMMENT ON COLUMN "comments"."parent_id" IS '父评论ID';

COMMENT ON COLUMN "comments"."user_id" IS '评论人ID';

COMMENT ON COLUMN "comments"."news_id" IS '评论新闻ID';

COMMENT ON COLUMN "comments"."updated_at" IS '修改时间';

COMMENT ON COLUMN "comments"."created_at" IS '创建时间';

COMMENT ON COLUMN "comment_likes"."comment_id" IS '点赞评论ID';

COMMENT ON COLUMN "comment_likes"."user_id" IS '点赞用户ID';

COMMENT ON COLUMN "comment_likes"."created_at" IS '创建时间';

COMMENT ON COLUMN "comment_likes"."updated_at" IS '修改时间';

ALTER TABLE "user_fans" ADD FOREIGN KEY ("follower_id") REFERENCES "users" ("id");

ALTER TABLE "user_fans" ADD FOREIGN KEY ("followed_id") REFERENCES "users" ("id");
...
ALTER TABLE "collections" ADD FOREIGN KEY ("news_id") REFERENCES "news" ("id");
